...
% 训练 SVM 模型
if length(classes) <= 2
    Mdl = fitcsvm(X_train_selected, y_train, 'KernelFunction', 'linear', 'Standardize', true);
else
    t = templateSVM('KernelFunction', 'linear', 'Standardize', true);
    Mdl = fitcecoc(X_train_selected, y_train, 'Learners', t);
end
...
% 保存结果
temp = ['基于Neighborhood Rough Set特征选择与SVM',  xlsx_file_name(1:end-5), num2str(month(now)), num2str(day(now)), num2str(hour(now)), num2str(minute(now)), num2str(second(now)), '.mat'];
save(temp);
