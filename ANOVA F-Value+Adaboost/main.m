...
f_values = zeros(num_features, 1);
for i = 1:num_features
    p = anova1(X_train(:, i), y_train, 'off');
    f_values(i) = 1 / p; % 使用 1/p 作为 F 值 (越小的 p 值表示越显著的特征)
end
[~, sorted_idx] = sort(f_values, 'descend');
...
% 显示结果
disp('Selected Features using ANOVA F-Value:');
disp(top_features);
disp(['Accuracy using AdaBoostM2: ', num2str(accuracy)]);
...
% 保存结果
temp = ['基于ANOVA F-Value特征选择与AdaBoostM2',  xlsx_file_name(1:end-5), num2str(month(now)), num2str(day(now)), num2str(hour(now)), num2str(minute(now)), num2str(second(now)), '.mat'];
save(temp);
