...
for i = 1:k
    ...
    % 训练 SVM 模型
    if length(classes) == 2
        Mdl = fitcsvm(X_selected(train_idx, :), y(train_idx), 'KernelFunction', 'linear', 'Standardize', true);
    else
        Mdl = fitcecoc(X_selected(train_idx, :), y(train_idx), 'Learners', templateSVM('KernelFunction', 'linear', 'Standardize', true));
    end
...
% 保存结果
temp = ['基于Relieff的特征选择与SVM', xlsx_file_name(1:end-5), num2str(month(now)), num2str(day(now)), num2str(hour(now)), num2str(minute(now)), num2str(second(now)), '.mat'];
save(temp);
