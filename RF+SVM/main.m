...
svmMdl = fitcecoc(X_train_selected, y_train, 'Learners', t);
y_pred = predict(svmMdl, X_val_selected);
...
temp = ['基于随机树的特征选择与SVM', xlsx_file_name(1:end-5), num2str(month(now)), num2str(day(now)), num2str(hour(now)), num2str(minute(now)), num2str(second(now)), '.mat'];
save(temp);
