...
mutual_info = zeros(num_features, 1);
for i = 1:num_features
    mutual_info(i) = mi(X_train(:, i), y_train); % 计算互信息
end
[~, mi_idx] = sort(mutual_info, 'descend');
top_features = mi_idx(1:5); % 选择最重要的前 5 个特征
...
% 显示结果
disp('Selected Features using Mutual Information:');
disp(top_features);
...
% 保存结果
temp = ['基于互信息特征选择与AdaBoost', xlsx_file_name(1:end-5), num2str(month(now)), num2str(day(now)), num2str(hour(now)), num2str(minute(now)), num2str(second(now)), '.mat'];
save(temp);
