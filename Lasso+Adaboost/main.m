...
if length(classes)<=2
    cvMdl = fitcensemble(X_selected, y, 'Method', 'AdaBoostM1', 'Learners', t, 'KFold', 10, 'NumLearningCycles', 50);
else
    cvMdl = fitcensemble(X_selected, y, 'Method', 'AdaBoostM2', 'Learners', t, 'KFold', 10, 'NumLearningCycles', 50);
end
...
% 保存结果
temp = ['基于Lasso的特征选择与AdaBoost', xlsx_file_name(1:end-5), num2str(month(now)), num2str(day(now)), num2str(hour(now)), num2str(minute(now)), num2str(second(now)), '.mat'];
save(temp);
