...
% FKNN分类器
Mdl = fitcknn(X_train_selected, y_train, 'NumNeighbors', 5);
y_pred = predict(Mdl, X_val_selected);
...
temp=['基于随机树的特征选择与FKNN',xlsx_file_name(1:end-5),num2str(month(now)),num2str(day(now)),num2str(hour(now)),num2str(minute(now)),num2str(second(now)),'.mat'];
save(temp);
